...
CONTROL_ID_MATCH = "IAM.5"
CONFIG_RULE_MATCH = "mfa-enabled-for-iam-console-access"
...
def lambda_handler(event, context):
    ...
    for finding in findings:
        ...
        is_iam5_control = control_id == CONTROL_ID_MATCH
        is_config_mfa_rule = config_rule_name == CONFIG_RULE_MATCH

        if not (is_iam5_control or is_config_mfa_rule):
            continue
        ...
        key = f"mfa-evidence/{datetime.utcnow().strftime('%Y-%m-%d-%H-%M-%S')}.json"
        ...
        sns.publish(
            ...
            Subject="MFA Compliance Finding Detected",
    ...
    if not processed:
        return {"status": "no_mfa_findings_matched"}
